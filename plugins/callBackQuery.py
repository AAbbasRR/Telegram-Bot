...
async def CallBackQueryGenerateCode(client, callback_query):
    ...
    if callback_query.data == "Generate":  # if clicked generate button
        ...
        rds_cache.set(f"{callback_query.from_user.id}_generate", json.dumps({"question": 0}))  # set user questions and answers in cache memory
        rds_cache.expire(f"{callback_query.from_user.id}_generate", 3600)  # set expire time for the user question key in cache
        ...
        try:
            user_answers = json.loads(rds_cache.get(f"{callback_query.from_user.id}_generate"))  # get user questions and answers in cache memory
            if user_answers['question'] == 3:  # if user in the sex question
                await client.send_chat_action(callback_query.from_user.id, "typing")  # Show the user that the robot is typing
                user_answers[str(user_answers['question'])] = callback_query.data  # set user answer in memory cache
                ...
                rds_cache.set(f"{callback_query.from_user.id}_generate", json.dumps(user_answers))  # update user answers in cache
                return None
        ...
        try:
            ...
            if user_answers['question'] > 8:  # If the user has a complete answer to all the questions
                ...
                if callback_query.data == "submit":  # if clicked submit button
                    rds_cache.delete(f"{callback_query.from_user.id}_generate")  # delete answers
...
please try again, to Generate the TD code, send the /generate command or use the «generate code» button""", reply_markup=BUTTONS['generate_code'])
                    ...
                    rds_cache.delete(f"{callback_query.from_user.id}_generate")
