...
async def message_question(client, message):
    ...
    try:
        with open(f"users/{message.from_user.id}ـgenerate.txt", "r") as file_read:
            ...
            if user_answers['question'] in [0, 5]:  # if user in country questions
                if len(message.text) < 3:  # if length of user message less than 3 character, send error Message
                    ...
                    user_answers[str(user_answers['question'])] = message.text.upper() if len(message.text) == 3 else message.text.capitalize()  # set user answer in file
                ...
                if len(message.text) != 6:  # if length of user message less than 6 or more than 6 character, send error Message
                    ...
                    try:
                        ...
                        user_answers[str(user_answers['question'])] = message.text  # set user answer in file
                        user_answers['question'] += 1  # In the file, find out which user is in the next question
                    except ValueError:
                        await message.reply(text=ERRORS['invalid_date_format'], quote=True)
                        return None
